...
###########################
# Name: pam_selection_and_sequence_logo.sh
# Author: Zhiyi Sun
#----------------
# Description: 
#----------------
...
awk -v f=$seq -v E=$end -v D=$distance -v O=$overhang '{if ($4==E && $5==D && $6==O) {print $NF > f; n=n+1}};END{print "Extracted "n" PAM sequences for the selected cut pattern"}' $input 
...
then
	...
	#	echo $bgf
		...
		weblogo --composition "$bgf" < $seq > $logo
...
awk 'BEGIN{print "Pos\tA\tC\tG\tT"};{if (NR>9) print last; last=$1"\t"$2"\t"$3"\t"$4"\t"$5}' $logodata > $matrix
...
#R CMD BATCH --no-save --quiet --slave "--args $matrix $pA $pC $pG $pT" /home/sunz/work/projects/CirclePAM/script/generate_PWM_seqlogo.r
